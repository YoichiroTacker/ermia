...
namespace ermia
{
  ...
  void transaction::Abort()
  {
    ...
    for (uint32_t i = 0; i < read_set.size(); ++i)
    {
      ...
      serial_deregister_reader_tx(&r->readers_bitmap); // downreadersbits?
    }
#endif
...
// 追加部分
  ...
  rc_t transaction::mvocc_commit()
  {
    ...
    check_backedge:
      ...
      else
      {
        ...
        if (volatile_read(successor_xc->owner) == xc->owner)
        { // myself
          continue;
        }
        auto successor_state = volatile_read(successor_xc->state);
  ...
  rc_t transaction::Update(IndexDescriptor *index_desc, OID oid, const varstr *k, varstr *v)
  {
    ...
    { // succeeded
      ...
      { // prev is committed (or precommitted but in post-commit now) head
#if defined(SSI) || defined(SSN) || defined(MVOCC)
    ...
    { // somebody else acted faster than we did
  ...
  rc_t transaction::ssi_read(dbtuple *tuple)
  {
    ...
    if (tuple_s1.asi_type() == fat_ptr::ASI_LOG)
    { // I'm T2
